...
global.mysql.query("SELECT * FROM config").then(function (rows) {
    ...
    let b321428 = global.protos.Block.encode({
        hash: "4cdbf2d1570dc930eef18b5e319a7735f635bfffc48f2dfc5a90c42abc3dc2d1",
        difficulty: 105,
        shares: 1,
        timestamp: 1542484613,
        poolType: 0,
        unlocked: false,
        valid: false,
        value: 10005676678443
    });
...
//        process.send({type: 'sendRemote', body: wsData.toString('hex')})
    request.post({url: global.config.general.shareHost, body: wsData.toString('hex')}, function (error, response, body) {
        console.log(error);
