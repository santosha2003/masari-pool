...
function getCoinPrice() {
	...
	global.support.https_get("https://tradeogre.com/api/v1/ticker/btc-etnx", function (res) {
		if (res != null && res.hasOwnProperty("price") && typeof(res.price) !== 'undefined') {
			price_btc = parseFloat(res.price);
			price_usd = parseFloat(price_btc*btc_usd);
			price_eur = parseFloat(price_btc*btc_eur);
			price_24h = parseFloat(btc_24h+100*(res.price-res.low)/res.low).toFixed(1);
		}
	});
	return ;
}
