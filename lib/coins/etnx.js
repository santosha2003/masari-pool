...
function Coin(data){
    ...
    this.getPortLastBlockHeader = function(port, callback){
		let getInfo;
		global.support.rpcPortDaemon(port, 'getinfo', [], function (body) {
            if (typeof(body) !== 'undefined' && body.hasOwnProperty('height') && body.hasOwnProperty('difficulty')){
                getInfo = body;
            } else {
                console.error(JSON.stringify(body));
            }
    ...
    this.cryptoNight = function(convertedBlob, port) {
        switch (port) {
            ...
            case 33081: return multiHashing.cryptonight(convertedBlob, 4);       // ETNX
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRSTAK2.exec(agent)) {
            ...
            let minorv = parseInt(m[2]) * 100;
            let minorv2 = parseInt(m[3]);
            if (majorv + minorv + minorv2 < 300) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.3.0+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        }
        return false;
    };
