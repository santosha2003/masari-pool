...
let activeSmartMiners = new Map();   // miners with algos/algos-perf
...
function retargetMiners() {
    ...
    function retargetMiner(miner) {
        if (miner.fixed_diff) {
            ...
            if (miner.difficulty * 20 < newDiff) {
    ...
    for (var [minerId, miner] of activeSmartMiners) retargetMiner(miner);
    for (var [minerId, miner] of activeMiners)      retargetMiner(miner);
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("retargetMiners() consumed " + elapsed + " ms for " + (activeSmartMiners.size + activeMiners.size) + " miners");
}
...
function removeMiner(miner) {
    ...
    if (miner.algos) {
      activeSmartMiners.delete(miner.id);
    } else {
...
function checkAliveMiners() {
    ...
    for (var [minerId, miner] of activeSmartMiners) if (miner.lastContact < deadline) removeMiner(miner);
    for (var [minerId, miner] of activeMiners)      if (miner.lastContact < deadline) removeMiner(miner);
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("checkAliveMiners() consumed " + elapsed + " ms for " + (activeSmartMiners.size + activeMiners.size) + " miners");
}
...
function templateUpdate(algo, repeating) {
    ...
    if (activePort) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        ...
        if (err === null) {
            ...
            if (repeating === true) setTimeout(templateUpdate, 200, algo, repeating);
        } else {
...
function setNewAlgoHashFactor(algo, algoHashFactor, check_height) {
    ...
    for (var [minerId, miner] of activeSmartMiners) {
        if (check_height) miner.trust.check_height = check_height;
        miner.sendNewJob();
    }

    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("setNewAlgoHashFactor() consumed " + elapsed + " ms for " + activeSmartMiners.size + " miners");
}
...
function setNewBlockTemplate(template) {
    ...
    if (algo === "") for (var [minerId, miner] of activeMiners) {
        if (isExtraCheck) miner.trust.check_height = height;
        miner.sendNewJob();
    }

    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("setNewBlockTemplate() consumed " + elapsed + " ms for " + activeMiners.size + " miners");
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.setAlgos = function(algos, algos_perf, algo_min_time) {
        ...
        else if ("cn/2" in algos_perf)                this.algos_perf[""]      = algos_perf["cn/2"];
        ...
        else if ("cryptonight/2" in algos_perf)       this.algos_perf[""]      = algos_perf["cryptonight/2"];
        else                                          return "algo_perf set should include cn, cn/1 or cn/2 hashrate";
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            ...
            if (!global.coinFuncs.isMinerSupportPortAlgo(port, miner.algos, activeBlockTemplate[algo].buffer[0])) return;
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            if (miner.algos) {
                activeSmartMiners.set(minerId, miner);
            } else {
            ...
            break;
        case 'getjob':
            miner = activeSmartMiners.get(params.id);
            if (!miner) miner = activeMiners.get(params.id);
            if (!miner) {
